...
db=mysql.connector.connect(host="localhost",port=3306,user='root',passwd="root",database="sl_project")
if(db):
...
if(key==1):
    ...
    string_1="create table absentees (Date int primary key not null,absentee_1 int,absentee_2 int,absentee_3 int,absentee_4 int,absentee_5 int,attendees int,"
...
class admin:
    ...
    def attendence(self):
        ...
        if(self.key==0):
            mycursor.execute(f"select * from absentees where Date='{date.today()}'")
            check=mycursor.fetchone()
            if (str(type(check))[8:16])=="NoneType":
...
#image labels 
#admin image label
img_admin=Image.open(r"Images_GUI/admin.png") #to load image
...
img_gen=Image.open(r"Images_GUI/QR_img.png") #to load image
...
#scan image label
img_scn=Image.open(r"Images_GUI/scan.jpg") #to load image
