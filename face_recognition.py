...
class Face_Recognition:
    ...
    def mark_attendance(self,i,r,n):
        with open('attendance.csv','r+',newline='\n') as f:
            ...
            #name_list=[]
            #already_in_file = set()
            #with open("Python-FYP-Face-Recognition-Attendence-System-master/attendance.csv","r") as f:
                #for line in f:
                    #already_in_file.add(line.split(",")[0])

            #if name_list not in already_in_file:
                #with open('Python-FYP-Face-Recognition-Attendence-System-master/attendance.csv', "a") as f:
                    #now = datetime.now()
                    #dtstring = now.strftime('%H:%M:%S')
                    #f.writelines(f'\n{name_list},{dtstring}')
